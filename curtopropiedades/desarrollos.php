<?php
	...
	if ($iPagesTotal > 1){
		if($iPage > 1) {
			$buffer_paging = '<li><a href="desarrollos/'. $lang .'/pagina/'. $back .'"><span>&larr;</span></a></li>';
		}
	    for($i=$from_page;$i<=$to_page;$i++){
	       if ($iPage == $i)
	          $buffer_paging .= '<li class="active"><span>' . $iPage . '</span></li>';
	       else
	          $buffer_paging .= '<li><a href="desarrollos/'. $lang .'/pagina/'. $i .'">'. $i .'</a></li>';
	    }
	}

	if($iPage < $iPagesTotal) {
		$buffer_paging .= '<li><a href="desarrollos/'. $lang .'/pagina/'. $next .'"><span>&rarr;</span></a></li>';
	}
