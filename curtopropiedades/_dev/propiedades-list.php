<?php
	...
	if (isset($zona) and $zona!='' and $zona!='todas') {
		...
		$fltr_zona_id = '';
		$fltr_zona_slug = str_replace('ñ', 'n', $_REQUEST['zona']);
		$fltr_zona_slug = friendlyUrl($fltr_zona_slug);
		$fltr_zona_name = $zona;
		$fltr_zona_orig = $zona;

		$sql = str_replace('4=4', '(
			lower(localidad) like "%'. strtolower($fltr_zona_orig) .'%" 
			or lower(pais) like "%'. strtolower($fltr_zona_orig) .'%" 
			or lower(provincia) like "%'. strtolower($fltr_zona_orig) .'%" 
			or lower(zona) like "%'. strtolower($fltr_zona_orig) .'%")', 
			$sql);
	} else {
	...
	/*if ($_POST['mode']=='propiedades-search') {
		...
		$url = $static_url ."propiedades/$lang/tipo-operacion/$fltr_op_slug/tipo-propiedad/$fltr_prop_slug/zona/". urlencode($fltr_zona_orig) ."/pagina/1";
